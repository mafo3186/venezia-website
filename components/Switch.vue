<template>
<button class="fixed top-8 right-4 flex justify-center items-center w-12 h-12 rounded-full bg-gray-200 bg-opacity-30 backdrop-blur-md"> &lt;- </button>
        <!-- <label
        style="z-index: 80!important;"
        @click="$emit('toggle-nav')"
  class="absolute right-[0.5em] top-[0.5em] z-40 cursor-pointer h-[3em] w-[6em] rounded-full bg-[hsl(0,0%,7%)] shadow-[0px_2px_4px_0px_rgb(18,18,18,0.25),0px_4px_8px_0px_rgb(18,18,18,0.35)]"
>
  <span
...
const { toggleNav } = defineEmits(["toggle-nav"]);
</script>